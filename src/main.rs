...
use adapters::rtk::{CommandSurface, adapter_contract_id, command_surface, command_surface_report};
#[cfg(test)]
...
if [ -z "$completion_file" ]; then
    printf 'xuva-plan: missing completion attestation path\n' >&2
    exit 1
fi
completion_staging="${completion_file}.staging"
cancel_token=
...
fn version_probe_arguments(tool: &str) -> Option<&'static [&'static str]> {
