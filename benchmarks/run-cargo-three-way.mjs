...
const variants = { raw: ["cargo.exe", ["check"]], native_rtk: [settings.nativeRtk, ["cargo", "check"]], rtk_wad_auto: [settings.wad, ["cargo", "check"]] };
const samples = [];
...
const policy = { schema_version: 1, evidence: [{ key: "cargo:check", raw_median_ms: summary.raw.median_ms, candidate_median_ms: summary.native_rtk.median_ms, token_savings_percent: summary.raw.o200k_tokens === 0 ? 0 : ((summary.raw.o200k_tokens - summary.native_rtk.o200k_tokens) / summary.raw.o200k_tokens) * 100, sample_count: summary.raw.runs }] };
