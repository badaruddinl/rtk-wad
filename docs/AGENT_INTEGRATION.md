...
XUVA is a dispatcher, not a replacement hook implementation. Its adapters
...
| Gemini CLI | `xuva agent hook gemini` | `rtk init -g --gemini` |
| GitHub Copilot | `xuva agent hook copilot` | `rtk init --copilot` |
...
2. Review the generated hook registration. Replace only the exact command
   `rtk hook <agent>` with `xuva agent hook <agent>`.
...
'{"tool_name":"Bash","tool_input":{"command":"git status"}}' |
  xuva agent hook claude
```

With stock RTK v0.43.0, the successful response contains
`updatedInput.command` with `xuva git status`. The same probe has been
