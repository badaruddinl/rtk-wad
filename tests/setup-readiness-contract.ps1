...
$temporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("rtk-wad-setup-readiness-" + [guid]::NewGuid())
$temporaryBase = [System.IO.Path]::GetTempPath()
$previousState = $env:RTK_WAD_STATE_DIR
$previousPath = $env:Path
$previousNativeRtk = $env:RTK_WAD_NATIVE_RTK_PATH
...
try {
    ...
    if (Test-Path -LiteralPath (Join-Path $env:RTK_WAD_STATE_DIR "setup-transaction-v1.json")) {
    ...
    if ($NativeRtk) { $env:RTK_WAD_NATIVE_RTK_PATH = $NativeRtk }
    ...
    $env:RTK_WAD_NATIVE_RTK_PATH = $previousNativeRtk
