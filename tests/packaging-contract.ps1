...
try {
    ...
    & $install -Destination $destination -TokenizerRoot $tokenizerRoot
    Assert-Condition (Test-Path -LiteralPath $target) "fresh install did not create the WAD launcher"
    Assert-Condition (Test-Path -LiteralPath (Join-Path $tokenizerRoot "Scripts\python.exe")) "fresh WAD install did not provision the tokenizer dependency"
    ...
    try { & $install -Destination $destination -SkipTokenizer } catch { $reinstallRejected = $true }
    ...
    & $install -Destination $destination -Force -SkipTokenizer
    ...
    try { & $install -Destination $tokenizerFailureDestination -TokenizerPython $missingPython } catch { $tokenizerFailureRaised = $true }
    Assert-Condition $tokenizerFailureRaised "missing tokenizer runtime did not fail the WAD install"
    ...
    $failedSafely = $false
    try { & $install -Destination $destination -Force -Source $missingSource -SkipTokenizer } catch { $failedSafely = $true }
